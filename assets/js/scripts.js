...
document.addEventListener('DOMContentLoaded', function () {
  ...
  form.addEventListener('submit', function (event) {
      ...
      // Create a reminder notification entry
      const reminderHTML = `
          <div class="notification-item">
              <p><strong>Reminder:</strong></p>
              <p>Hello, ${name}!</p>
              <p>You have an appointment for your ${make} on <strong>${date}</strong>.</p>
              <p>Service(s) chosen: ${services.join(', ')}</p>
          </div>
      `;

      // Append the reminder to the dropdown
      notificationDropdown.innerHTML += reminderHTML;

      // Show an alert or any confirmation message
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (user) {
    // User is logged in, show profile icon with username and Logout option
    authIcon.innerHTML = `
    ...
    // User is not logged in, show profile icon with Login/Sign Up that links to a combined page
    authIcon.innerHTML = `
      <a href="#" class="profile-icon-link"><i class="fas fa-user-circle"></i></a>
      <div class="profile-dropdown" style="display: none;">
